	...
	public long getNumberOfDataSets(SearchDTO searchDTO) {
		try {
			...
			CardinalityAggregationBuilder cardinalityAggregationBuilder = AggregationBuilders
					.cardinality("aggs_number_of_datasets").precisionThreshold(100).field("uri");
...
//                geoDistanceSortBuilder.setNestedPath("spatial");
	...
	private void updateAbsoluteCountOfAllFilters(SearchDTO searchDTO, String uri) {
		Arrays.stream(searchDTO.getFilters())
				.forEach(filterDTO -> filterDTO.getValues()
						.forEach(v -> v.getCount()
								.setAbsolute(calculateTheAbsoluteCount(searchDTO, uri, filterDTO.getSearchField(),
										filterDTO.getSearchField().equals("themes")
	...
	public FilterDTO getTopFiltersThatContain(SearchDTO searchDTO, String uri, String filterGroupTitle,
			String containsText) {
		try {
			...
			if (!optionalFilterDTO.isPresent())
				return null;
...
//                geoDistanceSortBuilder.setNestedPath("spatial");
	...
	private FilterDTO getIssueDateFilter() throws IOException {

		// TODO: Current state: max and min return wrong values. Maybe the best approach
		// is to filter datasets by years first, e.g. 2018 (124) 2019 (43). This can be
		// provided and a list (maybe instead of a slider) will be shown in the ui.

		// https://www.elastic.co/guide/en/elasticsearch/client/java-api/current/_metrics_aggregations.html#java-aggs-metrics-max

		MinAggregationBuilder minAggBuilder = AggregationBuilders.min("minIssued").field("issued");
		SearchRequest searchRequest = new SearchRequest();
		searchRequest.indices(es_index);
		SearchSourceBuilder searchSourceBuilder = new SearchSourceBuilder();
		// searchSourceBuilder = generateTopQuery(searchDTO, searchField, containsText);
		searchSourceBuilder.aggregation(minAggBuilder);
		searchRequest.source(searchSourceBuilder);
		SearchResponse searchResponse = restHighLevelClient.search(searchRequest, RequestOptions.DEFAULT);
		Aggregation minAgg = searchResponse.getAggregations().getAsMap().get("minIssued");
		if (minAgg != null && minAgg instanceof ParsedMin) {
			ParsedMin min = (ParsedMin) minAgg;
			System.err.println(min.getValueAsString());
			// 0001-01-01T00:00:00.000Z -> Incorrect
		}

		// max

		MaxAggregationBuilder maxAggBuilder = AggregationBuilders.max("maxIssued").field("issued");
		searchRequest = new SearchRequest();
		searchRequest.indices(es_index);
		searchSourceBuilder = new SearchSourceBuilder();
		// searchSourceBuilder = generateTopQuery(searchDTO, searchField, containsText);
		searchSourceBuilder.aggregation(maxAggBuilder);
		searchRequest.source(searchSourceBuilder);

		searchResponse = restHighLevelClient.search(searchRequest, RequestOptions.DEFAULT);
		Aggregation maxAgg = searchResponse.getAggregations().getAsMap().get("maxIssued");
		if (maxAgg != null && maxAgg instanceof ParsedMax) {
			ParsedMax max = (ParsedMax) maxAgg;
			System.err.println(max.getValueAsString());
			// 2020-11-02T00:00:00.000Z -> In the future ...
		}

		RangeDTO rangeDTO = new RangeDTO();
		rangeDTO.setGte("2018");
		rangeDTO.setLte("2019");

		return FilterDTO.builder().filterGroupTitle("Issue Date").searchField("issued").hasExternalLink(false)
				.hasStaticValues(true).selectedRangeValues(rangeDTO).build();
	}
	...
	private FilterDTO getFilterListOfLicenses(SearchDTO searchDTO, String uri) throws IOException {
		...
		return FilterDTO.builder().filterGroupTitle("License").hasExternalLink(true).hasStaticValues(false)
	...
	private void updateRelativeCount(SearchDTO searchDTO, String uri, String searchField, List<ValueDTO> values) {
		values.forEach(v -> v.getCount().setRelative(calculateTheRelativeCount(searchDTO, uri, searchField,
	...
	private void updateAbsoluteCount(SearchDTO searchDTO, String uri, String searchField, List<ValueDTO> values) {
		values.forEach(v -> v.getCount().setAbsolute(calculateTheAbsoluteCount(searchDTO, uri, searchField,
				searchField.equals("themes") ? themeConfiguration.getRevMap().get(v.getValue()) : v.getValue())));
	}
...
//            only searchQuery and searchIn are important for generating filter list
	...
	private QueryBuilder getContainsTextQuery(String searchField, String containsText) {
		MatchQueryBuilder query = QueryBuilders.matchQuery(removeKeywordFromSearchField(searchField), containsText)
				.analyzer(AnalysisRegistry.DEFAULT_ANALYZER_NAME);
	...
	private List<QueryBuilder> getOrderBy(SearchDTO searchDTO) {
		...
		if (selectedOrderValue.equals("relevance"))
			return ret;
	...
	private QueryBuilder getRangeFilterQuery(FilterDTO filterDTO) {
		...
		if (!selectedRangeValues.getGte().equals("-1"))
			range.gte(selectedRangeValues.getGte());
		if (!selectedRangeValues.getLte().equals("-1"))
			range.lte(selectedRangeValues.getLte() + "||+1d");
		return range;
	}
	...
	private QueryBuilder getFilterQuery(FilterDTO filterDTO) {
		...
		if (filterDTO.getSearchField().contains("."))
			return QueryBuilders.nestedQuery(calculateNestedPath(filterDTO.getSearchField()), filterQueryBuilder,
					ScoreMode.Avg);
	...
	private QueryBuilder getSearchKeyQuery(String searchKey, String[] searchIn) {
		if (searchKey == null || searchKey.isEmpty()) {
			...
			if (searchIn.length > 0)
				...
				fieldNames = Arrays.asList("title", "title_de", "description", "description_de", "keywords",
						"keywords_de");
